...
export default function EditAdmin({ user }: { user: UserType }) {
    const [open, setOpen] = useState(false);
    const [message, clientAction] = useFormState(submitInvite, undefined);
    const { register } = useForm<UserType>({ defaultValues: user });
    ...
    async function submitInvite(prevState: any, formData: FormData) {
        ...
        const res = await fetch(process.env.NEXT_PUBLIC_API_URL + '/users/update-admin/' + user._id, {
            method: 'POST',
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button variant="outline">Edit Admin</Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    <DialogTitle>Edit Admin Invite</DialogTitle>
                    <DialogDescription>
                        Edit and save when you&apos;re done.
                    </DialogDescription>
                </DialogHeader>
                <form action={clientAction} className="grid gap-4">
                    ...
                    <div className="flex items-center gap-8">
                        ...
                        <Select {...register("role")} name="role" defaultValue={user.role}>
                            ...
                            <SelectContent>
                                {
                                    ["ADMIN", "MANAGER", "USER"].map((role, index) => (
                                        <SelectItem key={index} value={role}>{capitalize(role)}</SelectItem>
                                    ))
                                }
                            </SelectContent>
                        </Select>
                    ...
                    <div className="flex items-center gap-8">
                        ...
                        {/* <Select {...register("plantId")} defaultValue={typeof user.plantId === "object" ? user.plantId._id : user.plantId} name="plantId">
                            <SelectTrigger className="col-span-3 py-4">
